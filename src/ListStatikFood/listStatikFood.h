...
boolean isIdxEffListStatik(ListStatik l, IdxType i)
{
    ...
    return (isIdxValidListStatik(l, i)) ? ((!isEmptyFood(ELMT_LIST_STATIK(l, i)))) : false;
}
...
ListStatik readFileMakananListStatik(char namaFile[])
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // Untuk panjang dan lebar makanan
        PANJANG(m) = strToInt(currentWord);
        ADVWORD();
        LEBAR(m) = strToInt(currentWord);
        ADVWORD();
...
void CATALOG(ListStatik l)
{
    ...
    for (int i = 0; i < listLengthStatik(l); i++)
    {
        ...
        printf(" ----------- ");
        printf("(%d,%d)", PANJANG(ELMT_LIST_STATIK(l, i)), LEBAR(ELMT_LIST_STATIK(l, i)));
        printf("\n");
    }
}
