...
void olahInventory(Simulator *sim, Akar resep, ListStatik daftarMakanan, TIME *realTime, NOTIF_STACK *NS)
{
    ...
    PANJANG(hasil) = PANJANG(ELMT_LIST_STATIK(daftarMakanan, searchID(daftarMakanan, INFO_TREE(resep))));
    LEBAR(hasil) = LEBAR(ELMT_LIST_STATIK(daftarMakanan, searchID(daftarMakanan, INFO_TREE(resep))));
